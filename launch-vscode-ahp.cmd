...
rem ============================================================
rem AHP 远控启动器：
rem   1. 生成/读取 agent host 连接 token（%USERPROFILE%\.copilot-bridge\token.txt）
rem   2. 后台启动 copilot-bridge daemon（8765，已运行则跳过）
...
rem  2. daemon 日志：%USERPROFILE%\.copilot-bridge\daemon.log
...
powershell -NoProfile -Command "if (-not (Get-NetTCPConnection -LocalPort 8765 -State Listen -ErrorAction SilentlyContinue)) { Start-Process -FilePath cmd.exe -ArgumentList '/c','cd /d %PROJECT_DIR% && npm run dev >> %BRIDGE_DIR%\daemon.log 2>&1' -WindowStyle Hidden; Write-Host 'daemon 已后台启动' } else { Write-Host 'daemon 已在运行（8765 端口占用）' }"
